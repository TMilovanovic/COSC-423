...
class SystemSimulator extends Thread {
	...
	// Used to store information to create a Gantt chart
	private final GanttChart chart = new GanttChart();
	...
	public void run() {
		...
		// loop while we have jobs left to be scheduled or scheduler has jobs
		// scheduled
		while (jobsRemainToBeSubmitted || myScheduler.hasJobs()) {
			...
			myScheduler.makeRun(); // the next Job should start running but
			...
			try {
				Condition jobCondition = myScheduler.getRunningJob().getMyCondition();
				jobCondition.await();
	...
	public void exit() {
		...
		terminatingJob.getMyCondition().signal(); // This should release the OS
													// to do its thing
		singleThreadMutex.unlock(); // N.B. this code is only executed by a Job
									// thread
	}
