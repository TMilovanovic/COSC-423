...
class Submittor extends Thread {
	...
	public void run() {
		...
		for (String jobDesc : myJobDescs) {
			...
			int delay = 0; // msec delay until this Job is submitted to the
							// kernel
			 ...
			 * 
			 */
			st = new StringTokenizer(jobDesc); // another string tokenizer to
												// parse the jobDesc
			...
			System.out.println(Thread.currentThread().toString() + "notifying the readyQ");
			
			// create jobs and add them to the Operating System
			mySystem.AddNewProcess(id, burstDescription, myWorkCreator.createWork());
		}
		mySystem.noMoreJobsToSubmit(); // let system know that no more jobs are
										// coming
	}
